...
const GameSession: React.FC = () => {
  ...
  useEffect(() => {
    if (!location.state?.room || !location.state?.character || !location.state?.party) {
      return; 
    }
    
    // If state is passed via navigate (from previous season), use it
    if (location.state.existingState) {
       setGameState(location.state.existingState);
  ...
  const toggleAgendaItem = (issueId: string) => {
    ...
    setGameState(updatePlatform(gameState, newAgenda, gameState.playerState.program));
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
             {getAvailableActions(gameState.playerState.role, victoryType).map(action => (
               <button
                 ...
                 className={`bg-slate-900 border border-slate-800 p-5 rounded-xl text-left hover:border-amber-600 hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed group relative overflow-hidden ${
                    action.type === 'GOVERNANCE' ? 'ring-1 ring-amber-500/30' : 
                 ...
                 <div className="relative z-10">
                   <div className="flex justify-between items-start mb-2">
                     <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${
                        action.type === 'CAMPAIGN' ? 'bg-amber-900/50 text-amber-400' :
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {group.demands.map(demandId => {
                        ...
                        return (
                          <span key={demandId} className={`text-[10px] px-1.5 py-0.5 rounded border ${
                            isAddressed 
