...
import { getGameRoom } from '../services/firestoreService';
  ...
  useEffect(() => {
    const initRoom = async () => {
      if (location.state?.room) {
        ...
        // Fetch from Firestore if direct navigation
        try {
          const fetchedRoom = await getGameRoom(id);
          if (fetchedRoom) {
            setRoom(fetchedRoom);
          } else {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {archetypes.map(arch => (
                <button
                  ...
                  className={`p-4 rounded-xl border text-left transition-all ${selectedArchetype?.id === arch.id
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {parties.map(party => (
                <button
                  ...
                  className={`relative p-4 rounded-xl border text-left transition-all overflow-hidden ${selectedParty?.id === party.id
